...
bool Socket::select ( Socket * new_socket, int nId, fd_set * fIn, fd_set * fOut, fd_set * fErr, timeval * tTime ) const
{
	new_socket->m_sock = ::select( nId, fIn, fOut, fErr, tTime);
...
int Socket::recv ( char input[MAXRECV] ) const
{
	char buf [ MAXRECV + 1 ];
...
// 0 == Receives, 1 == Sends, 2 == Both
bool Socket::shutdown (int type)
{
	int status = ::shutdown ( m_sock, type );
